...
const DocumentList = ({ userRole }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      setError('')
      const [docsResponse, classesResponse] = await Promise.all([
      ...
      // Handle classes response with multiple possible structures
      let classesData = []
      if (classesResponse.data.data && Array.isArray(classesResponse.data.data)) {
      ...
      } else if (classesResponse.data.classes && Array.isArray(classesResponse.data.classes)) {
